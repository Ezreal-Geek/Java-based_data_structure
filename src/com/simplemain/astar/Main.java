...
public class Main {
    ...
    static void search() {
        ...
        for (boolean finish = false; !finish && !heap.isEmpty(); ) {
            ...
            {
                final Point neXPnt = new Point(point.x + d[0], point.y + d[1]);
                if (neXPnt.x >= 0 && neXPnt.x < MAX_PNT.x && neXPnt.y >= 0 && neXPnt.y < MAX_PNT.y) {
                    char e = MAP[neXPnt.x][neXPnt.y];
                    ...
                    final Data inQueueData = heap.find(neXPnt);
                    ...
                    {
                        double h = h(neXPnt);
                        Data neXData = new Data(neXPnt, data.g + 1, h, data);
                        heap.add(neXData);
                    }
                }
            }
...
//		return hPoXEuclidianDistance(pnt);
//		return hManhattanDistance(pnt);
     ...
     * 欧式距离平方,大于等于实际值
     */
    static double hPoXEuclidianDistance(Point pnt) {
